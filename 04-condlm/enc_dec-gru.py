...
class Seq2SeqWithGRU(nn.Module):
    def __init__(self, nwords_src, nwords_trg, embed_size, hidden_size, dropout_p=0.5):
        ...
        self.decoder_gru = nn.GRU(embed_size + hidden_size, hidden_size, batch_first=True)
        ...
        for _ in range(max_len):
            ...
            for seq, score, hidden in sequences:
                if len(seq) > 0 and seq[-1] == eos_trg:
                    ...
                    continue
                trg_tensor = torch.LongTensor([seq[-1]]).unsqueeze(0).to(src.device) if len(seq) > 0 else torch.LongTensor([sos_trg]).unsqueeze(0).to(src.device)
                trg_embedded = self.embedding_trg(trg_tensor)
                attention_weights = self.attention(hidden[-1], encoder_outputs, src_mask)
                context = torch.bmm(attention_weights.unsqueeze(1), encoder_outputs)
                gru_input = torch.cat([trg_embedded, context], dim=2)
                output, hidden = self.decoder_gru(gru_input, hidden)
                ...
                for i in range(beam_width):
                    ...
                    candidate = (seq + [word_idx], score - word_prob, hidden)
...
EMBED_SIZE = 256  # 임베딩 사이즈 증가
HIDDEN_SIZE = 512  # 히든 사이즈 증가
model = Seq2SeqWithGRU(len(w2i_src), len(w2i_trg), EMBED_SIZE, HIDDEN_SIZE, dropout_p=0.5).to(device)
...
# Training loop
max_test_bleu = 0.0
for epoch in range(100):  # 에폭 수 조절
